jest.mock('@angular/router');

import { TestBed } from '@angular/core/testing';
import { HttpResponse } from '@angular/common/http';
import { HttpClientTestingModule } from '@angular/common/http/testing';
import { ActivatedRouteSnapshot, Router } from '@angular/router';
import { of } from 'rxjs';

import { IClientes, Clientes } from '../clientes.model';
import { ClientesService } from '../service/clientes.service';

import { ClientesRoutingResolveService } from './clientes-routing-resolve.service';

describe('Service Tests', () => {
  describe('Clientes routing resolve service', () => {
    let mockRouter: Router;
    let mockActivatedRouteSnapshot: ActivatedRouteSnapshot;
    let routingResolveService: ClientesRoutingResolveService;
    let service: ClientesService;
    let resultClientes: IClientes | undefined;

    beforeEach(() => {
      TestBed.configureTestingModule({
        imports: [HttpClientTestingModule],
        providers: [Router, ActivatedRouteSnapshot],
      });
      mockRouter = TestBed.inject(Router);
      mockActivatedRouteSnapshot = TestBed.inject(ActivatedRouteSnapshot);
      routingResolveService = TestBed.inject(ClientesRoutingResolveService);
      service = TestBed.inject(ClientesService);
      resultClientes = undefined;
    });

    describe('resolve', () => {
      it('should return IClientes returned by find', () => {
        // GIVEN
        service.find = jest.fn(id => of(new HttpResponse({ body: { id } })));
        mockActivatedRouteSnapshot.params = { id: 123 };

        // WHEN
        routingResolveService.resolve(mockActivatedRouteSnapshot).subscribe(result => {
          resultClientes = result;
        });

        // THEN
        expect(service.find).toBeCalledWith(123);
        expect(resultClientes).toEqual({ id: 123 });
      });

      it('should return new IClientes if id is not provided', () => {
        // GIVEN
        service.find = jest.fn();
        mockActivatedRouteSnapshot.params = {};

        // WHEN
        routingResolveService.resolve(mockActivatedRouteSnapshot).subscribe(result => {
          resultClientes = result;
        });

        // THEN
        expect(service.find).not.toBeCalled();
        expect(resultClientes).toEqual(new Clientes());
      });

      it('should route to 404 page if data not found in server', () => {
        // GIVEN
        jest.spyOn(service, 'find').mockReturnValue(of(new HttpResponse({ body: null as unknown as Clientes })));
        mockActivatedRouteSnapshot.params = { id: 123 };

        // WHEN
        routingResolveService.resolve(mockActivatedRouteSnapshot).subscribe(result => {
          resultClientes = result;
        });

        // THEN
        expect(service.find).toBeCalledWith(123);
        expect(resultClientes).toEqual(undefined);
        expect(mockRouter.navigate).toHaveBeenCalledWith(['404']);
      });
    });
  });
});
